    ...
    public int[] relativeSortArray(int[] arr1, int[] arr2) {
        ...
        while(second<arr2.length && first<arr1.length){
            ...
            if(arr1[first]==arr2[second]){
                ...
                // note that the search space for element `arr2[second]` is from `first` to the end of the arr1, because before the index `first` array is 
                // already relatively sorted, we just need to check in remaining array.
